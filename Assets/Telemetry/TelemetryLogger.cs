...
public class TelemetryLogger : MonoBehaviour
{
    ...
    public static void LogShotWhileCooling(float ammoPercentAtShot, string weapon, Vector3 shotPosition)
    {
        TelemetryEvent data = default;
        data.eventType = "FirstShotAfterCooling";
        data.fromAttribute = weapon;
        data.numberAttribute = ammoPercentAtShot;
        data.toPosition = shotPosition;

        _instance.LogTelemetry(data);
    }
    public static void LogClicksBeforeCanShoot(int clicks, string weapon, Vector3 clickPosition)
    {
        TelemetryEvent data = default;
        data.eventType = "ClicksWhenCan'tShoot";
        data.fromAttribute = weapon;
        data.numberAttribute = clicks;
        data.toPosition = clickPosition;

        _instance.LogTelemetry(data);
    }

    public static void LogTotalPistolShots(float totalShots)
    {
        TelemetryEvent data = default;
        data.eventType = "TotalPistolShots";
        data.numberAttribute = totalShots;

        _instance.LogTelemetry(data);
    }
    public static void LogTotalShotgunShots(float totalShots)
    {
        TelemetryEvent data = default;
        data.eventType = "TotalShotgunShots";
        data.numberAttribute = totalShots;

        _instance.LogTelemetry(data);
    }
    public static void LogTotalDiscShots(float totalShots)
    {
        TelemetryEvent data = default;
        data.eventType = "TotalDiscShots";
        data.numberAttribute = totalShots;

        _instance.LogTelemetry(data);
    }
    public static void LogTotalBurstShots(float totalShots)
    {
        TelemetryEvent data = default;
        data.eventType = "TotalBurstShots";
        data.numberAttribute = totalShots;
    ...
    /*
    public static void LogGunAccuracy(string weapon, float accuracy)
    {
        TelemetryEvent data = default;
        data.eventType = "GunAccuracy";
        data.toAttribute = weapon;
        data.numberAttribute = accuracy;

        _instance.LogTelemetry(data);
    }
    */

    public static void LogRPresses(float totalRPresses)
    {
        TelemetryEvent data = default;
        data.eventType = "TimesRwasPressed";
        data.numberAttribute = totalRPresses;
